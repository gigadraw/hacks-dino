(function () {
  ...
  function fireBullet() {
    const bulletHeights = [-30, 0, 30]; // Y offsets for the 3 floors (top, middle, bottom)

    bulletHeights.forEach(offset => {
      const bullet = document.createElement('div');
      bullet.className = 'dino-bullet';

      const top = gameContainer.getBoundingClientRect().top + tRex.yPos + 30 + offset; // Adjust the vertical position for each floor
  ...
  window.updateBullets = updateBullets; // Make the updateBullets function globally accessible
...
menu.innerHTML = `
  ...
  <button onclick="window.updateBullets()">🔫 Súng F</button> <!-- Fixed onclick -->
`;
